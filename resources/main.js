...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // ========== SISTEMA DE ARMAZENAMENTO PERSISTENTE COM FILESYSTEM ==========
    
    // Caminho do banco de dados (arquivo JSON persistente)
    const DB_FILE = 'calendar_database.json';
    ...
    async function initDatabase() {
        ...
        try {
            ...
            await Neutralino.filesystem.readFile(`./storage/${DB_FILE}`);
    ...
    async function loadDatabase() {
        try {
            if (isNeutralino) {
                const content = await Neutralino.filesystem.readFile(`./storage/${DB_FILE}`);
                return JSON.parse(content);
            } else {
                // Fallback para localStorage
                const data = localStorage.getItem('calendar_database');
                return data ? JSON.parse(data) : {};
            }
        } catch (error) {
            console.warn('⚠️ Erro ao carregar banco de dados, usando novo:', error);
            return {};
        }
    }
    ...
    async function saveDatabase(database) {
        try {
            if (isNeutralino) {
                ...
                // Fallback para localStorage
                localStorage.setItem('calendar_database', JSON.stringify(database));
    ...
    // Salvar dados específicos no banco
    ...
    // Obter dados específicos do banco
